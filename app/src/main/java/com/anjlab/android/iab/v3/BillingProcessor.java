...
public class BillingProcessor extends BillingBase {
	...
	public boolean purchase(Activity activity, String productId, String purchasePayload) {
		return purchase(activity, productId, purchasePayload, Constants.PRODUCT_TYPE_MANAGED);
	}

	public boolean subscribe(Activity activity, String productId, String purchasePayload) {
		return purchase(activity, productId, purchasePayload, Constants.PRODUCT_TYPE_SUBSCRIPTION);
	}

	private boolean purchase(Activity activity, String productId, String purchasePayload, String purchaseType) {
		...
		if (resultCode == Activity.RESULT_OK && responseCode == Constants.BILLING_RESPONSE_RESULT_OK) {
			...
			try {
				...
				if (developerPayload == null)
					developerPayload = "";
				boolean purchasedSubscription = developerPayload.startsWith(Constants.PRODUCT_TYPE_SUBSCRIPTION);
